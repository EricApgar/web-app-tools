...
# -----------------------------
class CounterWidget:
    def __init__(self, store: Store, name: str, title: str, period_seconds: float = 0.1) -> None:
    ...
    def on_toggle_click(self) -> None:
        ...
        if c.running:
            ...
            self.counter_label.text = f'Counter: 0'
...
with ui.row().classes('gap-6'):
    CounterWidget(store=store, name='counter_a', title='Counter A', period_seconds=0.1)
    CounterWidget(store=store, name='counter_b', title='Counter B', period_seconds=0.1)
